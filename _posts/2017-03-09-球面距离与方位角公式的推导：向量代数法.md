...
\begin{align*}
    \vec{\tau_1} &= \boldsymbol{OP} \times \vec{n_1} = \boldsymbol{OP} \times \left( \boldsymbol{OQ} \times \boldsymbol{OP} \right) \\\\\\
    \vec{\tau_2} &= \boldsymbol{OP} \times \vec{n_2} = \boldsymbol{OP} \times \left( \boldsymbol{ON} \times \boldsymbol{OP} \right)
\end{align\*}

其中，

\begin{align*}
    \boldsymbol{OP} &= \left( \cos\varphi_1 \cos\lambda_1 \, \cos\varphi_1 \sin\lambda_1 \, \sin\varphi_1 \right)\\\\\\
    \boldsymbol{OQ} &= \left( \cos\varphi_2 \cos\lambda_2 \, \cos\varphi_2 \sin\lambda_2 \, \sin\varphi_2 \right)\\\\\\
    \boldsymbol{ON} &= \left( 0, \, 0, \, 1\right)
\end{align\*}

易知$ \left | \boldsymbol{OP} \right | = \left |\boldsymbol{OQ} \right | = \left |\boldsymbol{ON} \right | = 1$
...
\begin{align*}
    \vec{\tau_1} \cdot \vec{\tau_2} &= \left[ \boldsymbol{OP} \times \left( \boldsymbol{OQ} \times \boldsymbol{OP} \right) \right] \cdot
                \left[ \boldsymbol{OP} \times \left( \boldsymbol{ON} \times \boldsymbol{OP} \right) \right]\\\\\\
            &= \boldsymbol{OP}
\end{align\*}
